  ...
  return (
    <main className="flex h-full items-center justify-center bg-surface px-6">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-surface-card border border-surface-border rounded-xl p-6">
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-surface-raised border border-surface-border rounded-lg px-3 py-2.5 text-sm text-text focus:border-accent outline-none transition" />
            </div>
        ...
        <p className="mt-4 text-center text-sm text-text-muted">No account? <Link href="/register" className="text-accent-hover hover:underline">Create one</Link></p>
      </div>
    </main>
  );
}
